...
If the `SHOW_ERRORS` configuration setting is set to `true`, full error detail will be shown in the browser if an error or exception occurs. If it's set to `false`, a generic message will be shown using the [App/Views/404.html](App/Home/Views/404.html) or [App/Views/500.html](App/Home/Views/500.html) views, depending on the error.
